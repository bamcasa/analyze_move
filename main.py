...
def trans(img):
    ...
    change_img = cv2.cvtColor(out, cv2.COLOR_BGR2GRAY)
    blurred = cv2.blur(change_img, (5, 5), 0)
    ...
    MIN_AREA = 5000
    MIN_WIDTH, MIN_HEIGHT = 1.0, 1.5
    MIN_RATIO, MAX_RATIO = 0.5, 1.75
...
cap = cv2.VideoCapture('real_fall.mp4')
...
while (cap.isOpened()):
    ret, frame = cap.read()
    if ret:
        frame = cv2.resize(frame, None, fx=scale_down, fy=scale_down, interpolation=cv2.INTER_LINEAR) #축소
        frame = trans(frame)
